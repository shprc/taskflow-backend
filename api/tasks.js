// api/tasks.js — Multi-user tasks (GET all / POST upsert / DELETE)
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    // Accept both camelCase (listName) and snake_case (list_name) from frontend
    const resolvedListName = t.listName || t.list_name || (t.category === 'people' ? 'Team' : t.category === 'projects' ? 'General' : 'Personal Actions');
    const { error } = await supabase.from('tf_tasks').upsert({
      ...
      is_archived: t.is_archived || t.isArchived || false,
    }, { onConflict: 'id' });
    if (error) return res.status(500).json({ error: error.message });
    return res.status(200).json({ success: true });
  }

  // DELETE — hard delete a task by id
