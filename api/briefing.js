...
module.exports = async (req, res) => {
  ...
  if (!userId) return;
  const { tasks, prompt: rawPrompt, mode } = req.body;
  if (!tasks && !rawPrompt) return res.status(400).json({ error: 'tasks or prompt required' });
  const { apiKey, context } = await getAIConfig(userId);
  if (!apiKey) return res.status(500).json({ error: 'No AI API key configured' });

  // Raw prompt mode — used by detectUpdate and other internal callers
  if (rawPrompt) {
    try {
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` },
        body: JSON.stringify({ model: 'gpt-4o-mini', messages: [{ role: 'user', content: rawPrompt }], temperature: 0.1, max_tokens: 300 })
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error?.message || 'OpenAI error');
      return res.status(200).json({ briefing: data.choices[0].message.content.trim() });
    } catch (err) {
      console.error('Briefing raw prompt error:', err);
